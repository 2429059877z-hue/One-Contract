...
- 素材来源：马也提供的保密/竞业一体化模板、京衡所修订版离职/保密/竞业协议、v1 协商解除审查素材（employment-termination-doctrine/cases/antipatterns）、劳动人事合同库。
...
- **条款联动冲突直接落笔**：同一交易内条款口径冲突且文本已写明事实时，属 `confirmed` 直接落笔统一口径，不以“请确认最终口径”类批注代替正文修订。见 `references/review-doctrine.md` 与 `references/equity-transfer-pilot.md`。
...
- **涉税给建议不给断言**：股权包 `equity-transfer-pilot.md`、`module_catalog.json`、`trigger_catalog.json` 中涉税措辞统一改为建议口径（如“建议按实际对价如实申报”），不作断言。
...
- 涉税事项改为实质专业判断口径：直接给出基于现行规则的判断（如“以实际对价如实订立，避免被税务机关按公平交易原则核定补税”），删除“请咨询税务专业人员”“需税务复核”式免责套话；只有确实超出合同审查判断范围的事项才说明原因并提示专项核实，且全稿不重复。同步修订 `references/equity-transfer-pilot.md`、`assets/knowledge_v2/module_catalog.json`、`trigger_catalog.json`。
